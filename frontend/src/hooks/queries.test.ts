...
vi.mock("../api/client", () => ({
  api: {
    ...
    getTopologyPositions: vi.fn().mockResolvedValue([]),
    getMetricsDevices: vi.fn().mockResolvedValue({ devices: [] }),
    getMetricsHistory: vi.fn().mockResolvedValue({ history: [] }),
    getNotifications: vi.fn().mockResolvedValue([]),
    getHealthSummary: vi.fn().mockResolvedValue({}),
    analyzeHealth: vi.fn(),
    dismissNotification: vi.fn(),
    getDocSections: vi.fn().mockResolvedValue({ sections: [] }),
    getRacks: vi.fn().mockResolvedValue([]),
    getRack: vi.fn().mockResolvedValue({ id: 1, name: "test", items: [] }),
...
describe("topology query hooks", () => {
  it("useTopologySvg does not fetch when disabled", () => {
    const { result } = renderHook(() => useTopologySvg("dark", "isometric", "unifi", "auto", true, false), { wrapper });
