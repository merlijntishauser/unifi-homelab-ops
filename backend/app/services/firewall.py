...
def _policy_to_rule(policy: FirewallPolicy, group_lookup: dict[str, FirewallGroup]) -> Rule:
    ...
    return Rule(
        ...
        source_matching_target=policy.source_matching_target,
        destination_matching_target=policy.destination_matching_target,
        destination_web_domains=list(policy.destination_web_domains),
        destination_web_matching_type=policy.destination_web_matching_type,
        destination_app_ids=list(policy.destination_app_ids),
...
def get_zones(credentials: UnifiCredentials) -> list[Zone]:
    ...
    log.info("unifi_fetch_zones", zone_count=len(result), network_count=len(network_lookup))
    return result
...
def get_rules(credentials: UnifiCredentials) -> list[Rule]:
    ...
    log.info("unifi_fetch_rules", rule_count=len(rules), group_count=len(group_lookup))
    return rules
...
def get_zone_pairs(credentials: UnifiCredentials) -> list[ZonePair]:
    ...
    log.info("zone_pairs_built", pair_count=len(result))
    return result
