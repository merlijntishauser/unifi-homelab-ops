...
- **Raised the Anthropic response budget from 4,096 to 16,384 tokens.** Thinking tokens are billed against the same budget, so a model that reasoned before answering could spend the old limit before finishing the findings array, truncating the JSON mid-structure. Billing is per token produced, so the headroom costs nothing on short replies
- **A reply cut off at the token limit now says so.** Truncated JSON previously reached the parser and surfaced as "Failed to parse AI response", pointing at the wrong problem -- the fix is a bigger budget, not a parsing change. Both providers are checked (Anthropic `stop_reason: max_tokens`, OpenAI `finish_reason: length`)
...
### Fixed
- **Wheel-zooming the topology diagram scrolled the page behind it.** The viewer called `preventDefault()` inside React's `onWheel`, which React attaches as a passive listener -- so the call did nothing but log `Unable to preventDefault inside passive event listener invocation` (127 errors in a short session). The listener is now registered natively with `{ passive: false }`
- **Fast wheel-zooming under-zoomed.** The handler read the current zoom from its render closure, so a burst of wheel events arriving before React re-rendered all compounded off the same stale value. Zoom and pan are now one state object updated functionally: five wheel steps produce 161% (1.1^5) rather than one step's worth
- **`no-connection-state` no longer dominates the security score.** It fired once per allow rule at high severity, on every rule that does not set a connection state -- so a zone pair with five well-scoped rules scored 0/F on this finding alone, and a rule pinned to a single host scored the same as a wide-open one. It is hardening advice, and the advice is identical for every rule, so it is now a single low-severity finding per zone pair listing the rules it applies to (via `rule_ids`). Golden fixtures updated: the affected pairs move from grade C to B
- **Domain- and application-restricted firewall rules are no longer graded as unrestricted.** A rule matching specific web domains or applications was flagged high severity ("allows all protocols and ports without restriction") because the criteria narrowing it were not in the model at all. `Rule` now carries `destination_web_domains`, `destination_app_ids`, `destination_web_matching_type` and the `source`/`destination` matching-target pair, and the analyzer treats any of them as a restriction. The matching target is used as the general signal, so criteria that are not decoded into a list (regions, app categories) also stop reading as unrestricted
